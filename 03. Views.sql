...
SELECT  doctor.doctor_id, 'Dr.' || doctor.first_name || ' ' || doctor.last_name as Doctor, doctor.office_no , department.dept_id, department.dept_name, 
...
SELECT patient.patient_id, patient.first_name || ' ' || patient.last_name as patient, floor(months_between(trunc(sysdate),patient.dob)/12) as age, calculate_bmi(patient.weight, patient.height) as bmi,
        patient.gender, diagnosis.diagnosis_category, diagnosis.diagnosis_details, 'Dr.' || doctor.first_name || ' ' || doctor.last_name as case_doctor, 
        TO_CHAR(appointment.appointment_date,'DD-MON-YYYY HH24:MI') appointment_date
